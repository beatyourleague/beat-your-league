...
`make backtest` runs the live grading + publishes BOTH pages, `make backtest-early` the
preregistered arm, `make backtest-retired` the historical study. **The retired study is
published at `site/retired-backtest.html`**, generated by the same code with its own masthead
saying what it is: the repoint moved 53.5% and the -5670.6 cost line off every buyer-reachable
surface, and while the repoint was preregistered before any result existed (so it is not
laundering), "the unflattering number got harder to find" is a charge worth having no answer
to give. The two pages link each other and their figures never appear together.

**An adversarial review of the repoint (Aug 23 2026) confirmed it honest and found four real
defects, two of them introduced by the fix itself** — the reviewer verified the preregistration
commit predates the graded run, so the decision could not have been tuned to a 64.6% outcome:
- **My own caption code stated two falsehoods.** It branched on the MEAN residual, which
  cancels: errors of +12,+11,+8,−7,−12,−13 average to nothing and it called a wildly
  miscalibrated chart "tracking the diagonal", while a positive mean with one band below
  published "above at every bucket" about a picture whose LARGEST band (2,627 calls) sat below.
  It branches on per-band residuals now and says "5 of 6 bands" when that is what is true.
- **The chart drew outside its own frame.** The axis window was hardcoded 45–90%, sized for the
  retired study; the live top band (91.2%, interval to 95%) and its whole error bar rendered
  outside the SVG — a chart silently omitting its most extreme result. Derived from the points now.
- **Every wrapped bullet was published in halves.** The source is hand-wrapped at ~78 columns
  and the converter consumed only lines beginning with a bullet, so "week-18 resting is a
  different" and "population." shipped as separate blocks. `verify()` could not see it: the
  FIGURES were all present, the `<li>` COUNT still matched (halves land in `<p>`), and
  flattening the page rejoins them into text that reads correct — so the check compares source
  items against the `<li>` CONTENTS.
- **Two `verify()` checks silently disabled themselves** when the source reworded its verdicts
  or its grade heading — the opposite of what a guard should do when it stops understanding its
  input. Verdicts are now read from the calibration table's own last column, and a missing grade
  is a failure rather than a skip. Same shape as the chart rule: `_is_publishable_section` is an
  **allowlist**, because guessing "not a diagnostic" from prose fails OPEN — a diagnostic worded
  unexpectedly and placed first took the chart AND suppressed the real one (reproduced).
- Fixed alongside: three pages that still called the retired study "our public backtest" or
  linked it as "the fuller version" of a non-comparable number, and `make backtest` silently
  shrinking the graded window on a partial cache (a skipped season is fatal now — the frozen
  method fixes the window).
